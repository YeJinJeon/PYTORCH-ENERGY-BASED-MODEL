...
def sample_2d_data(dataset, n_samples):
    ...
    if dataset == '8gaussians':
        ...
        """
        (x1, x2) where x1^2 + x2^2 >= 10 in domain [-5,5]
        """
        low, high = -5, 5
        dataset = torch.empty(n_samples, 2)
        valid_sample_num = 0
        while valid_sample_num < n_samples:
            x1 = (low-high) * torch.rand(1) + high
            ele = 10-x1**2
            if ele >= 0:
                high_x2 = - torch.sqrt(ele)
                x2_1  = (low - high_x2) * torch.rand(1) + high_x2
                low_x2 = torch.sqrt(ele)
                x2_2 = (low_x2 - high) * torch.rand(1) + high
                dataset[valid_sample_num] = torch.cat((x1, x2_1), 0)
                dataset[valid_sample_num+1] = torch.cat((x1, x2_2), 0)
                valid_sample_num += 2
            else:
                continue
        return dataset
        ...
        """
        (x1, x2) where sin(x1 + x2) >= 0.5 in domain [-5,5]
        """
        low, high = -5, 5
        valid_sample_num = 0
        dataset = torch.empty(n_samples, 2)
        while valid_sample_num < n_samples:
            x1 = (low-high) * torch.rand(1) + high
            x2 = (low-high) * torch.rand(1) + high
            if torch.sin(x1+x2) >= 0.5:
                dataset[valid_sample_num] = torch.cat((x1, x2), 0)
                valid_sample_num += 1
            else:
                continue
        return dataset
        ...
        """
        (x1, x2) where sin(x1^2 + x2^2) >= 0.5 in domain [-5,5]
        """
        low, high = -5, 5
        valid_sample_num = 0
        dataset = torch.empty(n_samples, 2)
